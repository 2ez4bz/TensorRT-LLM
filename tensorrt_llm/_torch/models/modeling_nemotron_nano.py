...
class NanoV2VLInputProcessor(BaseMultimodalInputProcessor, BaseMultimodalDummyInputsBuilder):
    def __init__(
        ...
        **kwargs,
        ...
        self._video_norm_mean = torch.tensor(getattr(config, "norm_mean", [0.5, 0.5, 0.5])).reshape(
            3, 1, 1
        )
        self._video_norm_std = torch.tensor(getattr(config, "norm_std", [0.5, 0.5, 0.5])).reshape(
            3, 1, 1
        )
    ...
    def _build_tubelet_separators(
        ...
        Matches vLLM's ``get_video_repl`` format for T > 1:
    ...
    def _process_video_prompts(
        ...
        for video_index, (num_tokens_per_frame, frame_separators) in enumerate(
            ...
            # NOTE: temporarily removed "This is a video:\n" prefix to match vLLM.
