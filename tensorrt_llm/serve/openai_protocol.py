...
class OpenAIBaseModel(BaseModel):
    ...
    # NOTE: set `extra="allow"`, since otherwise the `"forbid"` restriction applies "recursively"
    # to `CustomChatCompletionMessageParam` as well, which can be problematic if
    # `ChatCompletionRequest.messages` contains entries with e.g. `reasoning_content` as can be
    # returned in responses (see `ChatMessage` type definition).
