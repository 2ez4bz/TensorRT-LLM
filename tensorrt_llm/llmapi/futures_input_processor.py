"""Parallel input processing using ProcessPoolExecutor.
...
class FuturesInputProcessor(ParallelInputProcessor):
    """Manages a pool of worker processes for parallel input processing.

    Uses ProcessPoolExecutor for efficient worker management and proper
    Future support with concurrent.futures.as_completed().
    ...
    def __init__(
        ...
        # `ProcessPoolExecutor` spawns workers lazily, so submit no-op tasks to force all workers to
        # start and run their initializers.
        # NOTE: using more "proper" methods, such as a `ctx.Value` + `ctx.Lock` to have workers
        # report their own readiness at the end of the `initializer` function, is sadly not
        # feasible, as `initializer` is lazily run in processes when the first workload is
        # submitted.
        futures = [self._executor.submit(bool) for _ in range(num_workers)]
        cf.wait(futures)
